...
export default async function Home({ searchParams }: HomeProps) {
  ...
  const filteredItineraries = itineraries.filter((itinerary: ItineraryCardType) => {
    if (category && itinerary.moods[0].slug.current !== category && itinerary.moods[1]?.slug.current !== category) {
      return false;
    }
  ...
  return (
    <ClientOnly>
      ...
      <Container>
        <div className="pt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-8 overflow-x-hidden">
